...
#### Description  
Raw data for [Median Age of Weekly COVID-19 Cases, Weekly Deaths and Cases](https://ercbk.github.io/Indiana-COVID-19-Website/demographics.html#median-age-of-cases-tests-deaths) that shows daily confirmed COVID-19 cases by age group, gender, and county.
...
ca <- readxl::read_xlsx(glue("{rprojroot::find_rstudio_root_file()}/data/cases-age.xlsx"))
ca_var_defs1 <- c("Gender",
                 "",
                 "",
                 "",
                 "Age Group",
                 ...
                 "",
                 "",
                 "Daily confirmed COVID-19 cases by gender, age, and county",
                 "County")

ca_var_defs <- append(ca_var_defs1, rep("", 92))
...
tbl_summary(ca,
            missing = "ifany",
            missing_text = "Missing",
            include = c(-DATE, -LOCATION_ID)) %>% 
  modify_header(update = list(
    label ~ 'Variable')) %>% 
  modify_table_body(dplyr::mutate, defs = ca_var_defs) %>% 
  modify_header(defs = "Definition") %>% 
  # show_header_names()
  as_gt() %>%
  tab_options(table.align = "left") %>% 
